export const getRouting = async (lat1, lon1, lat2, lon2) => {
    try {
      ...
      const res = await fetch(
        `/api/bhuvan/routing?lat1=${lat1}&lon1=${lon1}&lat2=${lat2}&lon2=${lon2}`
      );
  ...
  export const getThematicData = async (lat, lon, year) => {
    try {
      // const base = import.meta.env.VITE_BASE_URL;
      const res = await fetch(
        `/api/bhuvan/thematic?lat=${lat}&lon=${lon}&year=${year}`
      );
  ...
  export const villageGeocoding = async (category) => {
    try {
      // const base = import.meta.env.VITE_BASE_URL;
      const res = await fetch(`/api/bhuvan/vg?village=${category}`);
 ...
 export const getEllipsoid = async (id) => {
  try {
    // const base = import.meta.env.VITE_BASE_URL;
    const res = await fetch(`/api/bhuvan/ellipsoid?id=${id}`);
  ...
  export const getFloodRunoff = async (catchmentId, outletLat, outletLon) => {
    try {
      // const base = import.meta.env.VITE_BASE_URL;
      const res = await fetch(
        `/api/bhuvan/floodrunoff?catchmentId=${catchmentId}&outletLat=${outletLat}&outletLon=${outletLon}`
      );
