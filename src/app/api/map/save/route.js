...
export async function POST(req) {
  ...
  try {
    ...
    const { view, wmsLayers, pbfLayers, geoJsonUrls, annotations } = mapState;

    const viewJSON = JSON.stringify(view).replace(/'/g, "''");
    const wmsJSON = JSON.stringify(wmsLayers).replace(/'/g, "''");
    const pbfJSON = JSON.stringify(pbfLayers).replace(/'/g, "''");
    ...
    const query = `
      ...
      INSERT INTO map_states (project_id, view_state, wms_layers, pbf_layers, geojson_urls, annotations)
      VALUES ('${projectId}', '${viewJSON}', '${wmsJSON}', '${pbfJSON}', '${geoJSON}', '${annJSON}');
    `;
